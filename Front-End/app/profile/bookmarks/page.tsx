...
export default function BookmarksPage() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const res = await api('/api/posts/bookmarked');
        setPosts(res.data);
      } catch (e) {
        console.error(e);
      }
  ...
  return (
    <>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        ...
        <h1 className="text-2xl font-bold mb-6">🔖 저장한 기록</h1>
        
        <div className="card">
          {posts.length === 0 ? (
            <p className="text-gray-500 text-center py-8">저장한 글이 없습니다.</p>
          ) : (
              ...
              {posts.map((post) => (
                <li key={post.post_id}>
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-400 mt-1">{post.author_nickname}</p>
                  </Link>
                </li>
              ))}
            </ul>
